...
const CreateTeam = () => {
    ...
    const fetchLeague = async () => {
      try {
        const response = await fetch(
          `http://localhost:3000/api/leagues/${params.leagueID}`
        );
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:3000/api/leagues/${params.leagueID}/create-team`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        router.push(`/league/${params.leagueID}`); // Redirect to the league page
      } else {
